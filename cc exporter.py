...
def get_nifi_down_or_up():
    nifi_cluster_urls = ['http://nifi-http.default.svc.cluster.local:8080/nifi-api/controller/cluster']
...
def get_nifi_metrics_processor( group_ids ):
    ...
    NIFI_API = "http://nifi-http.default.svc.cluster.local:8080/nifi-api/flow/process-groups"
    for id  in group_ids:
        url = f"{NIFI_API}/{id}" 
        response = requests.get(url)
        if response.status_code == 200:
            ...
            processer_url = 'http://nifi-http.default.svc.cluster.local:8080/nifi-api/processors'
