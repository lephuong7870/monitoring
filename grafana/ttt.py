...
##
nifi_status_invoke     = Histogram('nifi_status_invoke_api', 'NiFi Status Invoke', ['group_name' , 'id_processor' , 'link_url', 'status' , 'message' ]) 
nifi_status_processors = Histogram('nifi_status_processors_api', 'NiFi Status Processor', [ 'group_name',  'id_processor' , 'nameType' , 'status' , 'message' ])  
nifi_status_kafka      =  Histogram('nifi_status_kafka_connect', 'NiFi Status Kafka Connect', ['group_name',   'id_processor'  , 'topic' , 'broker' , 'nameType',  'status' ]) 
        ...
        if response.status_code == 200:
            ...
            for i in all_processors:
                ...
                if (status == 'ERROR' ) and ( nameType == "InvokeHTTP" ) :
                    ...
                    nifi_status_invoke.labels( nameGroup , id_processor , link_url, status , message).observe(duration)
                    ...
                    try:
                        ...
                        nameTopic = ''
                        broker    = ''
                    nifi_status_kafka.labels( nameGroup , id_processor , nameTopic , broker , nameType , status ).observe(duration)
       
                else:
                    pass  
                
                if status == 'ERROR' : 
                    nifi_status_processors.labels( nameGroup , id_processor , nameType , status , message   ).observe(duration)
                else:
                    nifi_status_processors.labels( nameGroup , id_processor , nameType , status , message   ).observe(duration)

        else:
            nifi_status_invoke.labels( '' , '' , '', '' , '').observe(duration)
            nifi_status_processors.labels( '' , '' , '' , '' , ''   ).observe(duration)
            nifi_status_kafka.labels( '' , '' , '' , '' , '', ''   ).observe(duration)
