...
def get_nifi_down_or_up():
    nifi_cluster_urls = ['http://127.0.0.1:51003/nifi-api/controller/cluster']
...
def get_nifi_metrics_processor( group_ids ):
    ...
    NIFI_API = "http://127.0.0.1:51003/nifi-api/flow/process-groups"
    for id  in group_ids:
        url = f"{NIFI_API}/{id}" 
        response = requests.get(url)
        if response.status_code == 200:
            ...
            processer_url = 'http://127.0.0.1:51003/nifi-api/processors'
...
while True:
    ...
    try:
       ...
       group_ids = []
    group_ids  = [ "ddcb79b2-d5b3-3a72-bebf-7538db57edc1" , "c3b7a254-da30-3abb-a07a-9789e26b38f6"]
